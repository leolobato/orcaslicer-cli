...
**`app/slicer.py`** — Slicing orchestration. Resolves profiles, writes them as temp JSON files, sanitizes the input 3MF (clamps invalid parameter values via `_CLAMP_RULES`), performs smart settings transfer from the 3MF onto the process profile, then shells out to `orca-slicer` CLI. Serialized to one concurrent slice via `asyncio.Semaphore(1)`. Customization detection uses the 3MF's own fingerprint at `different_settings_to_system[0]` as an allowlist — only keys the GUI recorded as user-customized are transferred onto the target process profile. When the fingerprint is absent or empty, nothing is transferred (matches OrcaSlicer GUI semantics when re-selecting a process preset). Keys starting with `filament_` or ending with `_filament` are still excluded even if listed in the allowlist.
...
- 3MF project settings use smart transfer driven by the 3MF's own `different_settings_to_system[0]` fingerprint (a semicolon-separated list of process keys the GUI recorded as user-customized when the file was saved). Only those keys are overlaid onto the target process profile. Keys the fingerprint doesn't declare are left at the target process profile's values. When the fingerprint is missing or empty, nothing is transferred. The `/slice` response includes `X-Settings-Transfer-Status` (`applied`, `no_customizations`, `no_3mf_settings`) and `X-Settings-Transferred` (JSON array of `{key, value, original}` objects when status is `applied`) headers. Per-filament customizations at `different_settings_to_system[2+]` are not yet honored — see `docs/slice-parity-todo.md`.
