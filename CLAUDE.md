...
- Multi-plate 3MFs: plate isolation runs in `cpp/src/slice_mode.cpp` step 10b. Computes the requested plate's plater-coords origin via the GUI formula (`PartPlateList::compute_origin` at `vendor/OrcaSlicer/src/slic3r/GUI/PartPlate.cpp:3765` — col/row stride is `bed_size * (1 + 1/5)`; cols is `compute_colum_count` at `PartPlate.hpp:38`), builds a `BuildVolume` shifted to that origin, and calls libslic3r's `Model::update_print_volume_state` (`vendor/OrcaSlicer/src/libslic3r/Model.cpp:687`). Off-plate instances get `print_volume_state != Inside` and would already be filtered by `Print::apply` via `is_printable()` (`Model.hpp:1335`, see `PrintApply.cpp:145`), but the headless binary also iterates `model.objects` directly in copies/arrange/auto_center, so we physically remove off-plate instances/objects and translate the surviving instances by `-plate_origin` to bed-local coords. After this step the model looks like a single-plate 3MF and the existing pipeline (`set_plate_origin(Zero)`, auto_center, arrange) is unchanged. Returns `invalid_plate_id` when `plate_id` is out of range and `empty_plate` when no instance falls inside the requested plate's volume.
